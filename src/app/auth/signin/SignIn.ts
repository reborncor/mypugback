...
import {
    checkThatPasswordsAreEqualsOrThrow, checkThatUserExistsOrThrow,
...
export const signIn = async  (req : Request, res : Response) =>{
    ...
    try{
        ...
        res.status(200).json({code : successCode, message : "Connexion avec succès", payload : userToUserResponse(user), token : generateAccessToken(user)});
    }catch (err : any){
...
const signInUser = async (username : string, password : string): Promise<User> => {
