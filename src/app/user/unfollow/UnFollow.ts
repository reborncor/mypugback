...
import {
    ...
    checkThatUserIsNotAlreadyFollow, checkThatUserNotFollowed,
...
const execute = async (userId : string, username: string): Promise<any> => {
    ...
    const userNotFollow = await FollowerRepository.findUserInFollwingList(currentUser.username, otherUser.username);
    checkThatUserNotFollowed(userNotFollow);

    const follower : Follower = {
        _id : new ObjectId(otherUser._id) , username : otherUser.username
    }
    const following : Follower = {
        _id : new ObjectId(otherUser._id) , username : currentUser.username
    }
    ...
    return {
        code : 0, message :"Nouvel utilisateur unfollow"
    }
